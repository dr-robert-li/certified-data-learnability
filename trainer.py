...
class Trainer():
    ...
    def train_recovery(self, epoch, model, optimizer, poison_model, eta, project=True):
        model.train()
        origin_param = {}
        ...
        for i, (images, labels) in enumerate(self.data_loader[self.target]):
            images, labels = images.to(device, non_blocking=True), labels.to(device, non_blocking=True)
            start = time.time()
            log_payload = self.train_recovery_batch(images, labels, model, poison_model, origin_param, eta, optimizer, proj=project)
            end = time.time()
            time_used = end - start
            if self.global_step % self.log_frequency == 0:
                display = util.log_display(epoch=epoch,
    ...
    def train_recovery_batch(self, images, labels, model, poison_model, origin_param, eta, optimizer, proj):
        model.zero_grad()
        optimizer.zero_grad()
        if isinstance(self.criterion, torch.nn.CrossEntropyLoss) or isinstance(self.criterion, models.CutMixCrossEntropyLoss):
            logits = model(images)
